...
class Mdl_content extends CI_Model {
	...
	function add_news_config($img_name){
		$cok = $this->cek_data('berita','title',$this->input->post('title'));
		// if ($cok==0){
			if($this->input->post('title')) {
				...
				// print_r($record_detail);die();
					$this->db->insert('berita',$record_detail);
					$tes = 1;
					return $tes;			
			}else {
				$tes = 0;
	...
	function edit_news_config($id_news_config){
		$a = $this->db->select('*')->where('id',$id_news_config)->get('berita')->result();
		...
		if($this->input->post('id')){
			...
			$this->db->where('id',$id_news_config)->update('berita',$record_detail);
			return 1;		
		}else{
			return 0;
		}
	...
	function get_all_news_active(){
		$sql = "SELECT * FROM berita WHERE status='A'";
	...
	function get_news_byid($id){
		...
		$query = $this->db->select('*')->where($condition)->get('berita')->result();
		return $query;
	}
