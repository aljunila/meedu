            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
						...
						<div class="col-sm-5">
							<select class="form-control" name="user_group_id" id="ugi">
								...
								<?php	if(!isset($new_enduser)){ 
									if($enduser->user_group_id=='1'){echo 'selected="selected"'; }
								}?>
								>Admin Sekolah</option>
								<option value="5" 
								<?php	if(!isset($new_enduser)){ 
									if($enduser->user_group_id=='5'){echo 'selected="selected"'; }
								}?>
						...
						<div class="col-sm-5">
							<select class="form-control" name="institution_id" >
								...
								<?php		
									...
									if(!isset($new_enduser)){
								?>	
									<option value="<?php echo $as->id; if($as->id == $enduser->institution_id){echo 'selected="selected"';} ?>"><?php echo $as->name; ?></option>
								<?php
									}
								?>
							</select>
					...
					<div class="form-group" id="yayasan_id">
						...
						<div class="col-sm-5">
							<select class="form-control" name="yayasan_id" >
								<?php
									foreach($all_yayasan as $ys){
								?>
									<option value="<?php echo $ys->id; ?>"><?php echo $ys->name; ?></option>
								<?php		
									}
									if(!isset($new_enduser)){
								?>	
									<option value="<?php echo $ys->id; if($ys->id == $enduser->institution_id){echo 'selected="selected"';} ?>"><?php echo $ys->name; ?></option>
								<?php
									}
								?>
							</select>
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" placeholder="John Doe"  name="fullname" <?php if(!isset($new_enduser)){ echo 'value="'.$enduser->fullname.'"'; } ?> autofocus>
						</div>
					</div>							
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
						<input type="text" class="form-control" placeholder="nomor induk kependudukan" name="nik" <?php if(!isset($new_enduser)){ echo 'value="'.$enduser->nik.'" '; } ?>>
						</div>
					</div>
					...
					<label class="col-sm-2 control-label">Email</label>
						<div class="col-sm-5">
							<input type="email" class="form-control" place="account name" name="username" <?php if(!isset($new_enduser)){ echo 'value="'.$enduser->username.'" '; } ?>>
						</div>
					</div>
					<div class="form-group">
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-footer">
						                    ...
						                    <a href="<?php echo site_url('enduser/enduser_crud/'.$enduser->id.'-3'); ?>"><button type="button" class="btn btn-outline">Delete</button></a>
						                </div>
...
<script type="text/javascript">
    ...
    function selectedUgi(){
	    document.getElementById('ugi').addEventListener('change', function () {
		    if(this.value==1){
