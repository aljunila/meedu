...
class Mdl_content extends CI_Model {
	...
	function add_promo($img_name){
		$cok = $this->cek_data('promo','title',$this->input->post('title'));
		// if ($cok==0){
			if($this->input->post('title')) {
				$dp = str_replace('/', '-', $this->input->post('p_date'));
				$date_publish = date('Y-m-d', strtotime($dp));

				$du = str_replace('/', '-', $this->input->post('u_date'));
				$date_unpublish = date('Y-m-d', strtotime($du));

				$record_detail = array(
						'title' => $this->input->post('title'),
						'content' => $this->input->post('content'),
						'image' => $img_name,
						'publish_date' => trim($date_publish),
						'unpublish_date' => trim($date_unpublish),
				...
				// print_r($record_detail);die();
					$this->db->insert('promo',$record_detail);
					$tes = 1;
					return $tes;			
			}else {
				$tes = 0;
	...
	function edit_promo($id_news_config){
		$a = $this->db->select('*')->where('id',$id_news_config)->get('promo')->result();
		...
		if($this->input->post('id')){
			...
			$this->db->where('id',$id_news_config)->update('promo',$record_detail);
			return 1;		
		}else{
			return 0;
		}
	...
	function get_all_promo_active(){
		$sql = "SELECT a.*, b.name as category_name 
				FROM promo as a
				LEFT JOIN category_promo as b 
	...
	function get_category() {
		$sql = "SELECT * FROM category_promo WHERE status='A'";
		$query = $this->db->query($sql)->result();
		return $query;
	}

	function get_promo_byid($id){
		$condition = array('id' => $id);
		$query = $this->db->select('*')->where($condition)->get('promo')->result();
		return $query;
	}
