...
// PERLU DIUBAH
	function promo_crud($tes){
		$title_page='Management Promo';
		...
		$primary_id = $status[0];
		$last_url = 'promo/promo_crud/0-0';
		if(($cas >= '4') && ($cas !== '7')){
			$last_url = 'promo/promo_crud/0-4';		
		}
		...
		switch($cas){
			...
			case 1: // UNTUK ADD 
					// if($get_priviledge=='1'){
						$data['category'] = $this->mdl_content->get_category();
						$data['new_data'] = true;
						$this->menu_handler->set_menu('promo_detail','Input Promo Baru',$data,$menu_data['menu_row'],$menu_data['sub_menu_row']);
					// } else {
					// 	$this->menu_handler->error();
					// }
					break;
			case 2: // UNTUK EDIT
					if($get_priviledge==CA){
						$data['category'] = $this->mdl_content->get_category();
						$edit_school = $this->mdl_content->get_promo_byid($status[0]);
						$data['data'] = $edit_school[0];
						$this->menu_handler->set_menu('promo_detail','Edit Promo',$data,$menu_data['menu_row'],$menu_data['sub_menu_row']);
					} else {
						$this->menu_handler->error();
					} 	
					break;
    ...
    function populate_data(){
    	...
    	$res = $this->mdl_content->get_all_promo_active();
            ...
            <table id="tbl_data" class="table table-bordered table-striped" style="width:100%;">
              ...
              <tbody>';
				...
				<tr>
                  	...
                  	<td>'.$EndDate.'</td>
                    <td align="center">
                        <a class="btn btn-warning btn-xs" href="'.site_url('promo/promo_crud/'.$row->id.'-2').'"          rel="tooltip-top" title="Edit"><i class="fa fa-edit"></i></a>
	...
	function add_data(){
		...
		if($new_news_config==true){  // ADD PROJECT
			...
			$imgName = 'img_promo_'.time().'.jpg';
			$config['file_name']			= $imgName;
	        $config['upload_path']          = FCPATH.'data/promo/';
	        ...
	        if ( ! $this->upload->do_upload('image')){
				...
				if($conditi == "YOU DID NOT"){
					...
					$this->menu_handler->set_menu('promo_detail','news_config',$data,1,1);
				}
    ...
    function add_news_config($imgName){
		...
		if($get_priviledge=='1'){
			$result = $this->mdl_content->add_promo($imgName);
			$data['daftar'] = $this->mdl_content->get_all_promo_active();
			if($result == 1){
				$data['daftar'] = $this->mdl_content->get_all_promo_active();
				...
				$data['alert'] = 'alert alert-danger';
				$this->menu_handler->set_menu('promo_detail','Berita',$data,1,1);
			} else {
				$data['alert'] = 'alert alert-danger';
	...
	function edit_news_config($id_news_config, $status=''){
		$get_priviledge = $this->session->userdata('user_group_id');
			if($id_news_config =='save'){
				$result = $this->mdl_content->edit_promo($this->input->post('id'));
				if($result==1){
					$data['daftar'] = $this->mdl_content->get_all_promo_active();
