...
// PERLU DIUBAH
	function account_crud($tes){
		$title_page='Account Data';
		...
		$primary_id = $status[0];
		$last_url = 'account/account_crud/0-0';
		if(($cas >= '4') && ($cas !== '7')){
			$last_url = 'account/account_crud/0-4';		
		}
		$menu_data = $this->menu_handler->get_menu($last_url);
		$institution_id= $this->session->userdata('institution_id');
		$data['anggota'] =$this->mdl_content->get_all_anggota_active();
    ...
    function populate_data(){
    	$res = $this->mdl_content->get_all_parent_account();
            ...
            <table id="tbl_data" class="table table-bordered table-striped" style="width:100%;">
              <thead>
                <tr>
                  ...
                  <th style="width:15%;">Setup Biaya</th>
              ...
              <tbody>';
				$i=0;
				foreach ($res as $row){
  				$data = $this->mdl_content->get_all_account_byparent($row->id);
				$i++;
				$tables = $tables.'
				<tr>
					...
					foreach ($data as $get) {
						...
						<tr>
		                  	...
		                  	<td>'.$get->code.' '.$get->name.'</td>
		                  	<td>'.$get->description.'</td>
		                  	<td>Rp. '.number_format($get->nominal, 0, ',', '.').'
		                        <button class="btn bg-success btn-xs pull-right" onClick="prepare_setup('.$get->id.')"><i class="fa fa-retweet"></i></button>
		                    </td>
		                  	<td align="center">
		                        <button class="btn bg-teal btn-xs" onClick="prepare_edit_data('.$get->id.')"><i class="fa fa-edit"></i></button>
		                        <button class="btn btn-warning btn-xs" onClick="prepare_delete_data('.$get->id.')"><i class="fa fa-trash-alt"></i></button>
		                  	</td>
		                </tr>';
					}
					$tables = $tables.'
...
// TDK PERLU DIUBAH    
	...
	function populate_acc(){
		$categories= $this->mdl_content->get_all_parent_account();;
		...
		$table =$table. '<select class="form-control" name="a_parent_id" id="a_parent_id">
					...
					foreach($categories as $row){ 
						...
						$table= $table.'>'.$row->name.'</option>';
					}
		$table = $table.'</select>';
		$table= $table.'</div>
              <div class="input-group-addon">
                <a class="btn btn-info btn-xs" onClick="ajaxAddAcc()"><i class="fa fa-plus"></i></a>
              </div>
	...
	function add_acc(){
		$result = $this->mdl_content->add_acc();
		if($result == 1){
	...
	function setup_detail(){
    	$result= $this->mdl_content->get_setup($this->input->post('id'));
    	$data['item'] = $result[0];
    	echo json_encode($data);
    }
    ...
    function add_setup(){
		...
		if($get_priviledge=='1'){
			$result = $this->mdl_content->add_setup();
			if($result == 1){
					$status = $this->lang->line('msg_data_success_add');
					$alert = 'alert alert-success';
				} else if($result == 2){
	...
	function edit_setup(){
		...
		$data['new_subject'] = true;
		if($get_priviledge=='1'){
			$result = $this->mdl_content->edit_setup($this->input->post('id'));
			if($result == 1){
				$status = $this->lang->line('msg_data_success_edit');
				$alert = 'alert alert-success';
			} else if($result == 2){
